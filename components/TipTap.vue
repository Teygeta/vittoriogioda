...
import { Loader2, AlignCenter, AlignLeft, AlignRight, Bold, Code, Code2, Dot, Heading1, Heading2, Heading3, Italic, ListOrdered, MessageSquareQuote, Underline as UnderlineIcon } from 'lucide-vue-next'
...
<template>
  <div>
    <div v-if="editor" class="flex gap-1 p-2 my-2 border rounded">
      ...
      <button :class="{ 'is-active text-white': editor.isActive('heading', { level: 1 }) }"
        @click="editor.chain().focus().toggleHeading({ level: 1 }).run()">
        <Heading1 :size="25" />
      </button>
      <button :class="{ 'is-active text-white': editor.isActive('heading', { level: 2 }) }"
